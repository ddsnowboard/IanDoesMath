...
function handleResize(canvas) {
    console.log({
        'canvas html height': canvas.jGraph.height(),
        'canvas html width': canvas.jGraph.width(),
        'canvas height': canvas.jGraph[0].height,
        'canvas width': canvas.jGraph[0].width
    });
...
if (this.document) {
    ...
    onmessage = function(e) {
        ...
        var func = function(x) {
            return math.compile(jsonObj.func).eval({
                x: x
            });
        };
